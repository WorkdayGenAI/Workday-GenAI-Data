# Workday Data Reconciliation AI Agent
...
3. [Install and Verify Python](#3-install-and-verify-python)
4. [Install Application Dependencies](#4-install-application-dependencies)
5. [Start the Web Application](#5-start-the-web-application)
6. [Use the Application](#6-use-the-application)
7. [Discrepancy Classifications](#7-discrepancy-classifications)
8. [Dashboard and Report Output](#8-dashboard-and-report-output)
9. [Performance Guidelines](#9-performance-guidelines)
10. [Troubleshooting](#10-troubleshooting)
11. [Recommended Use in Workday Projects](#11-recommended-use-in-workday-projects)
...
| Python Libraries | `flask`, `pandas`, `openpyxl`, `xlsxwriter` |

---

## 3. Install and Verify Python

1. **Check whether Python is installed** — Open PowerShell and run:
   ```powershell
   python --version
   ```

2. **If Python is not installed** — Install Python from the [official Python website](https://www.python.org/downloads/) and select **Add Python to PATH** during installation.

3. **Restart PowerShell** — Close and reopen PowerShell after installation so PATH changes are refreshed.

4. **Validate again** — Run `python --version`. You should see a Python version number instead of a Microsoft Store alias or command-not-found message.

---

## 4. Install Application Dependencies

Navigate to the folder where the application file is saved, then install the required packages. Either install from CMD prompts or open **Visual Studio → New Terminal** and use the commands below.
...
Launch the app locally from PowerShell using Visual Studio or a similar tool. Open a new terminal and enter the commands below. Ensure the Python code file is saved in the correct folder and use the path accordingly.
...
| 3 | Run Reconciliation | Start processing. The app follows: **Load Data → Normalize → Match Keys → Compare Fields → Detect Issues → Generate Reports**. |
| 4 | Review Output | Use the dashboard and downloadable Excel report to review exceptions. |
...
The dashboard allows the reviewer to confirm whether processing is complete and quickly understand the reconciliation result. It displays:
...
The downloadable Excel reconciliation report includes at minimum:

- **Summary Reconciliation Report**
- **Detailed Discrepancy Report**
- **Full Record Dump** _(if enabled)_
- **Column Mismatch Analytics** _(if enabled)_
...
| Port already in use | Change the Flask port in the app run command or code, e.g. `app.run(port=5001)`. |
